...
class Formula:
    ...
    def _parse_prefix(string: str) -> Tuple[Formula, str]:
        ...
        if string[0] == '(':
            inner = string[1:]
            first, rest = Formula._parse_prefix(inner)
            if rest[0] in '&|':
                op = rest[0]
                rest = rest[1:]
            else:
            ...
            assert rest[0] == ']'
            return Formula(quant, var, stmt), rest[1:]
        else:
            if not (string[0] >= 'F' and string[0] <= 'T'):
                ...
                while i < len(string) and string[i].isalnum():
                    i += 1
                name = string[:i]
                if i < len(string) and string[i] == '(' and is_relation(name):
                    rest = string[i+1:]
                    args = []
                    if rest and rest[0] == ')':
                        rest = rest[1:]
